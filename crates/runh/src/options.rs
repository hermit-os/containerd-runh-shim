...
use crate::{error::Error, io::Io, utils, DefaultExecutor, LogFormat, Runh, Spawner};

// constants for log format
pub const JSON: &str = "json";
pub const TEXT: &str = "text";

// constants for runh global flags
...
// constants for runh-create/runh-exec flags
...
// constants for runh-kill flags
const ALL: &str = "--all";

// constants for runh-delete flags
const FORCE: &str = "--force";

// constant for command
pub const DEFAULT_COMMAND: &str = "runh";
...
/// Global options builder for the runh binary.
///
/// These options will be passed for all subsequent runh calls.
/// See <https://github.com/opencontainers/runc/blob/main/man/runc.8.md#global-options>
#[derive(Debug, Default)]
pub struct GlobalOpts {
    /// Override the name of the runh binary. If [`None`], `runh` is used.
    ...
    /// Use systemd cgroup.
    systemd_cgroup: bool,
    /// Timeout settings for runh command.
    ///
...
impl GlobalOpts {
    ...
    /// Default is `/run/runh`, or `$XDG_RUNTIME_DIR/runh` for rootless containers.
    ...
    // the `slice:prefix:name` form (e.g. `system.slice:runh:434234`).
    ...
    pub fn build(self) -> Result<Runh, Error> {
        self.args()
    }
    ...
    fn output(&self) -> Result<(PathBuf, Vec<String>), Error> {
        let path = self
            .command
            .clone()
            .unwrap_or_else(|| PathBuf::from("runh"));
...
mod tests {
    ...
    fn global_opts_test() {
        ...
        assert!(args.contains(&"/tmp/runh.log".to_string()));
        assert!(args.contains(&LOG_FORMAT.to_string()));
