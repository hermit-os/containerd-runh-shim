...
use crate::{
    common::{
        check_kill_error, create_io, create_runh, get_spec_from_request, receive_socket,
        CreateConfig, Log, ProcessIO, ShimExecutor, INIT_PID_FILE, LOG_JSON_FILE,
    },
    io::Stdio,
};

pub type ExecProcess = ProcessTemplate<RunhExecLifecycle>;
pub type InitProcess = ProcessTemplate<RunhInitLifecycle>;

pub type RunhContainer = ContainerTemplate<InitProcess, ExecProcess, RunhExecFactory>;

#[derive(Clone, Default)]
pub(crate) struct RunhFactory {}

#[async_trait]
impl ContainerFactory<RunhContainer> for RunhFactory {
    async fn create(
        &self,
        ns: &str,
        req: &CreateTaskRequest,
    ) -> containerd_shim::Result<RunhContainer> {
        ...
        let mut init = InitProcess::new(
            id,
            stdio,
            RunhInitLifecycle::new(runh.clone(), opts.clone(), bundle),
        );

        let config = CreateConfig::default();
        self.do_create(&mut init, config).await?;
        let container = RunhContainer {
            id: id.to_string(),
            bundle: bundle.to_string(),
            init,
            process_factory: RunhExecFactory {
                runtime: runh,
                bundle: bundle.to_string(),
                io_uid: opts.io_uid,
                io_gid: opts.io_gid,
    ...
    async fn cleanup(&self, _ns: &str, _c: &RunhContainer) -> containerd_shim::Result<()> {
        Ok(())
    }
}
...
impl RunhFactory {
    async fn do_create(&self, init: &mut InitProcess, _config: CreateConfig) -> Result<()> {
        ...
        let mut create_opts = runh::options::CreateOpts::new()
            .pid_file(&pid_path)
...
pub async fn runtime_error(bundle: &str, e: runh::error::Error, msg: &str) -> Error {
...
#[async_trait]
impl ProcessFactory<ExecProcess> for RunhExecFactory {
    async fn create(&self, req: &ExecProcessRequest) -> Result<ExecProcess> {
        ...
        Ok(ExecProcess {
            ...
            wait_chan_tx: vec![],
            console: None,
            lifecycle: Arc::from(RunhExecLifecycle {
...
#[async_trait]
impl ProcessLifecycle<InitProcess> for RunhInitLifecycle {
    ...
    async fn kill(
        ...
        self.runtime
            .kill(
                p.id.as_str(),
                signal,
                Some(&runh::options::KillOpts { all }),
            )
            .await
    ...
    async fn delete(&self, p: &mut InitProcess) -> containerd_shim::Result<()> {
        if let Err(e) = self
            ...
            .delete(
                p.id.as_str(),
                Some(&runh::options::DeleteOpts { force: true }),
            )
            .await
        {
    ...
    async fn ps(&self, p: &InitProcess) -> Result<Vec<ProcessInfo>> {
        let pids = self
            ...
            .ps(&p.id)
            .await
            .map_err(other_error!(e, "failed to execute runh ps"))?;
        Ok(pids
            .iter()
            .map(|&x| ProcessInfo {
                pid: x as u32,
...
impl RunhInitLifecycle {
    pub fn new(runtime: Runh, opts: Options, bundle: &str) -> Self {
...
#[async_trait]
impl ProcessLifecycle<ExecProcess> for RunhExecLifecycle {
    async fn start(&self, p: &mut ExecProcess) -> containerd_shim::Result<()> {
        ...
        let mut exec_opts = runh::options::ExecOpts {
            io: None,
...
impl Spawner for ShimExecutor {
    async fn execute(&self, cmd: Command) -> runh::Result<(ExitStatus, u32, String, String)> {
        let mut cmd = cmd;
        let subscription = monitor_subscribe(Topic::Pid)
            .await
            .map_err(|e| runh::error::Error::Other(Box::new(e)))?;
        let sid = subscription.id;
        let child = match cmd.spawn() {
            Ok(c) => c,
            Err(e) => {
                monitor_unsubscribe(sid).await.unwrap_or_default();
                return Err(runh::error::Error::ProcessSpawnFailed(e));
            }
        };
...
mod tests {
    ...
    use runh::error::Error::CommandFailed;
    use tokio::fs::remove_dir_all;

    use crate::{common::LOG_JSON_FILE, runh::runtime_error};
